...
export default function SignUp() {
  ...
  const getPasswordStrength = (pass: string) => {
    if (pass.length === 0) return { level: 0, label: '', color: 'transparent' };
    if (pass.length < 6) return { level: 1, label: 'Weak', color: '#EF4444' };
    if (pass.length < 10) return { level: 2, label: 'Medium', color: '#F59E0B' };
  ...
  return (
    <LinearGradient colors={['#0a0a1a', '#0f172a', '#0a0a1a']} style={styles.container}>
      <View style={styles.bgContainer} pointerEvents="none">
        <View style={[styles.glow, { width: 250, height: 250, top: -80, right: -80, backgroundColor: 'rgba(59,130,246,0.08)' }]} />
        <View style={[styles.glow, { width: 200, height: 200, bottom: 100, left: -100, backgroundColor: 'rgba(139,92,246,0.06)' }]} />
      </View>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.form}>
            <InputField
              icon={<User size={18} color={focusedField === 'name' ? '#60A5FA' : '#6B7280'} />}
            ...
            <InputField
              icon={<Lock size={18} color={focusedField === 'confirm' ? '#60A5FA' : '#6B7280'} />}
          ...
          {password.length > 0 && (
            <View style={styles.strengthContainer}>
              ...
              <View style={styles.strengthBars}>
                {[1, 2, 3].map((level) => (
                  <View key={level} style={styles.strengthBarBg}>
          ...
          <TouchableOpacity onPress={handleSignUp} disabled={isLoading} style={[styles.primaryButton, isLoading && styles.primaryButtonDisabled]} activeOpacity={0.8}>

...
Initialization used PostgreSQL's documented `--pwfile`, `--auth-host=scram-sha-256`, `--auth-local=scram-sha-256`, and `--data-checksums` controls. Start checks exact output from all four pinned tools. Because those read-only probes activate the WSL VM, start first proves PostgreSQL and the host port absent, terminates only the exact token-authorized `ThriveLens-R0` distro, and waits on host memory for three consecutive samples at the stricter maximum of the leased runtime and install floors (currently 2 GiB). It then reasserts the unchanged configuration lease and fingerprint, performs the full active-distro identity validation again, rechecks guest and host absence, and takes a fresh sample against the unchanged 1 GiB runtime floor immediately before the guarded `pg_ctl` attempt. Preflight similarly terminates only the exact project distro, settles at its action-specific floor, and reasserts stopped-distro identity, host-port absence, and a fresh memory sample before reporting `READY`. Neither path performs a global WSL shutdown, retries a start, lowers a threshold, or persists a configuration change.

`pg_ctl` uses a 30-second wait and binds only `127.0.0.1:55432`. Any post-start failure triggers bounded cleanup. Stop can report `STOPPED` or `ALREADY_STOPPED` only after exact binary-process and dedicated-port listener absence. Every forced distro termination additionally requires the held lifecycle mutex and an in-memory host-only token matching the canonical Lxss GUID/name/version/BasePath plus VHD file identity immediately before and after termination; lock failure or identity drift never falls back to unguarded termination. WSL 2.6.3 offers only name-targeted termination, so a hostile same-user/OS-control replacement in the irreducible precheck-to-command interval remains a documented residual that post-validation detects but cannot make atomic. The runtime test accepts a wrong-password result only when a bounded private `LC_ALL=C` stderr buffer exactly classifies PostgreSQL's `tl_bootstrap` password rejection, then proves the same server remains usable with the protected credential before stopping and verifying absence. See upstream [`initdb`](https://www.postgresql.org/docs/17/app-initdb.html) and [`pg_ctl`](https://www.postgresql.org/docs/17/app-pg-ctl.html).

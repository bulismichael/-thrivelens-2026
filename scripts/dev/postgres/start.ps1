...
try {
    ...
    $attempted=$true
    $start=Invoke-ThriveLensDistro -TimeoutSeconds 45 -Arguments @('/usr/sbin/runuser','-u','postgres','--',$p.PgCtl,'start','-D',$p.DataRoot,'-l','/dev/null','-o',$options,'-w','-t','30')
    ...
    if($attempted){
        ...
        try{$null=Stop-ThriveLensPostgresUnderLock}catch{$code='POSTGRES_START_CLEANUP_FAILED';$fatal=$true}
        try{Stop-ThriveLensDistroAndVerify;Assert-ThriveLensHostPortAbsent}catch{$code='POSTGRES_START_CLEANUP_FAILED';$fatal=$true}
    }
    elseif($wslTouched){
        try{Assert-ThriveLensWslAbsent;Assert-ThriveLensHostPortAbsent}catch{$code='POSTGRES_START_CLEANUP_FAILED';$fatal=$true}
        try{Stop-ThriveLensDistroAndVerify}catch{$code='POSTGRES_START_CLEANUP_FAILED';$fatal=$true}
        try{Assert-ThriveLensHostPortAbsent}catch{$code='POSTGRES_START_CLEANUP_FAILED';$fatal=$true}
    }

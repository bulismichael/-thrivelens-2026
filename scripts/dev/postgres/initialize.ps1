...
try {
    ...
    if($clusterState -ceq 'VALID'){
        ...
        Stop-ThriveLensDistroAndVerify -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock;Assert-ThriveLensHostPortAbsent
    ...
    foreach($directory in @('/var/lib/thrivelens',$parent,'/var/log/thrivelens','/var/log/thrivelens/postgresql',$logParent)){
        $exists=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/test','-e',$directory)
        if($exists.ExitCode -eq 1){$mkdir=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/install','-d','-o','postgres','-g','postgres','-m','0700',$directory);if($mkdir.ExitCode -ne 0){throw 'CLUSTER_DIRECTORY_CREATE_FAILED'}}
        elseif($exists.ExitCode -ne 0){throw 'WSL_PATH_MEASUREMENT_FAILED'}
    }
    Assert-ThriveLensLinuxPathPolicy
    $nonce=[guid]::NewGuid().ToString('N')
    $staging="$parent/.r0-staging-$nonce"
    $stale=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/find',$parent,'-mindepth','1','-maxdepth','1','-name','.r0-staging-*','-print','-quit')
    ...
    try {
        $pwCreate=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -StandardInput ($passwordValue+"`n") -Arguments @('/usr/bin/install','-o','postgres','-g','postgres','-m','0600','/dev/stdin',$distroPasswordFile)
    }
    finally { $passwordValue=$null }
    if($pwCreate.ExitCode -ne 0){throw 'DISTRO_PASSWORD_FILE_CREATE_FAILED'}
    $init = Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -TimeoutSeconds 120 -Arguments @(
    ...
    $unlink=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/rm','-f','--',$distroPasswordFile)
    $absent=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/test','!','-e',$distroPasswordFile)
    ...
    $null = Assert-ThriveLensLinuxTreePolicy -Root $staging -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock
    $control = Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/lib/postgresql/17/bin/pg_controldata',$staging)
    ...
    $size = Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/du','-sb',$staging)
    ...
    $finalAbsent=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/test','!','-e',$paths.DataRoot);if($finalAbsent.ExitCode -ne 0){throw 'CLUSTER_ACTIVATION_TARGET_PRESENT'}
    $activationAttempted=$true
    $move=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/mv','-T','--no-clobber','--',$staging,$paths.DataRoot)
    if($move.ExitCode -ne 0){throw 'CLUSTER_ACTIVATION_FAILED'}
    $sourceAbsent=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/test','!','-e',$staging);$finalExists=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/test','-e',$paths.DataRoot)
    ...
    if($null -ne $lifecycleLock -and $null -ne $cleanupIdentityToken){try{$null=Assert-ThriveLensWslCleanupIdentity -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock;$cleanupIdentityReady=$true}catch{$code='INITIALIZE_CLEANUP_IDENTITY_FAILED';$fatalCleanup=$true}}
    $containmentVerified=$code -cne 'WSL_GUARDED_COMMAND_CONTAINMENT_FAILED'
    if(-not $containmentVerified){
        # A bounded command may have left a guest child alive. Before touching
        # credentials or staging, retry exact old-token containment. If this
        # cannot be proved, preserve all state for explicit recovery.
        if($cleanupIdentityReady){
            try{Stop-ThriveLensDistroAndVerify -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock;Assert-ThriveLensHostPortAbsent;$containmentVerified=$true}
            catch{$code='INITIALIZE_GUARDED_CONTAINMENT_UNVERIFIED';$fatalCleanup=$true}
        }
        else{$code='INITIALIZE_GUARDED_CONTAINMENT_UNVERIFIED';$fatalCleanup=$true}
    }
    if($containmentVerified -and $cleanupIdentityReady -and $null -ne $distroPasswordFile){
        try{
            $unlink=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/rm','-f','--',$distroPasswordFile)
            $absent=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/test','!','-e',$distroPasswordFile)
            if($unlink.ExitCode -ne 0 -or $absent.ExitCode -ne 0){throw 'INITIALIZE_CREDENTIAL_CLEANUP_FAILED'}
        }
        catch{
            $credentialFailure=if($_.Exception.Message -match '^[A-Z0-9_]+$'){$_.Exception.Message}else{'INITIALIZE_CREDENTIAL_CLEANUP_FAILED'}
            $policy=Resolve-ThriveLensCleanupContainmentPolicy -FailureCode $credentialFailure -SameTokenContainmentReverified $false
            if($policy.RequiresRecontainment){
                $containmentVerified=$false
                try{
                    Stop-ThriveLensDistroAndVerify -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock
                    Assert-ThriveLensHostPortAbsent
                    $containmentVerified=$true
                    $policy=Resolve-ThriveLensCleanupContainmentPolicy -FailureCode $credentialFailure -SameTokenContainmentReverified $true
                }
                catch{$containmentVerified=$false;$code='INITIALIZE_GUARDED_CONTAINMENT_UNVERIFIED'}
            }
            if(-not $policy.AllowGuestCleanup){$containmentVerified=$false}
            if($containmentVerified){$code='INITIALIZE_CREDENTIAL_CLEANUP_FAILED'}
            $fatalCleanup=$true
        }
    }
    ...
    if($containmentVerified -and $cleanupIdentityReady -and $null -ne $staging -and -not $activationAttempted -and -not $activated){
        try{
            Assert-ThriveLensWslAbsent
            $exists=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/test','-e',$staging)
            if($exists.ExitCode -eq 1){$staging=$null}
            elseif($exists.ExitCode -ne 0){throw 'measurement'}
            if($null -ne $staging){$null=Assert-ThriveLensLinuxTreePolicy -Root $staging -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock}
            if($null -ne $staging){$cleanup=Invoke-ThriveLensGuardedDistro -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock -Arguments @('/usr/bin/python3','-c',@'
...
'@,$staging)
    ...
    if($cleanupIdentityReady -and ($wslTouched -or $mutated -or $activationAttempted)){
        ...
        try{Stop-ThriveLensDistroAndVerify -IdentityToken $cleanupIdentityToken -LifecycleLock $lifecycleLock}catch{$code='INITIALIZE_DISTRO_CLEANUP_FAILED';$fatalCleanup=$true}
